...
* `blackforestlabs/flux-3-video-t2v`
{% endhint %}
{% endcolumn %}

{% column width="33.33333333333334%" %}
<a href="https://aimlapi.com/app/blackforestlabs/flux-3-video-t2v" class="button primary">Try in Playground</a>
{% endcolumn %}
{% endcolumns %}

## Model Overview

FLUX 3 Video — a text-to-video model by Black Forest Labs that generates clips of 5 to 20 seconds in hd or fhd, with a synchronized audio track.
...
{% openapi-operation spec="flux-3-video-t2v" path="/v2/video/generations" method="post" %}
[OpenAPI flux-3-video-t2v](https://raw.githubusercontent.com/aimlapi/api-docs/refs/heads/main/docs/api-references/video-models/Black-Forest-Labs/flux-3-video-t2v.json)
...
generation = requests.post(url, headers=headers, json={'model': 'blackforestlabs/flux-3-video-t2v', 'prompt': 'Describe what you want the model to generate.', 'duration': 5, 'image_url': 'https://example.com/input.jpg'})
...
const generation = await fetch(url, {
  ...
  headers,
  body: JSON.stringify({
  "model": "blackforestlabs/flux-3-video-t2v",
