...
{% hint style="info" %}
In the examples below for JavaScript and Python, we use the [**REST API SDK**](../supported-sdks-old.md#rest-api). This approach works with all of our APIs, but it is not the only way to integrate. You can use [**other supported SDKs**](../supported-sdks-old.md), or make direct requests using **cURL** without any SDK.
{% endhint %}
