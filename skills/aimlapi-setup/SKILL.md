---
name: aimlapi-setup
description: Install and configure the aimlapi CLI so the coding agent you are running in (Claude Code, Codex, OpenCode, Cline, OpenClaude, OpenClaw, Hermes Agent, zero or ZCode) uses AI/ML API models. Use when the user asks to "set up AI/ML API", "connect to aimlapi", "use AI/ML API models", or points you to this file. Signs in through the browser (the user approves, no key is pasted), previews every config change with --dry-run, applies it after the user confirms, verifies it, and explains how to roll it back.
---
